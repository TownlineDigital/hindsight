...
export const api = {
  ...
  // Live "what's being analyzed right now" preview for the loading screen
  // (added 2026-07-09, direct user request) - see backend/job_files.
  // latest_frame_path's docstring. Returns {"path": null} (never throws)
  // when nothing's been sampled yet - JobProgress.jsx polls this while a job
  // is running and only fetches the actual image (via frameBlobUrl) when the
  // path changes, so idle polling ticks don't re-download anything.
  latestFrame: (jobId) => getJSON(`/jobs/${jobId}/latest-frame`),
